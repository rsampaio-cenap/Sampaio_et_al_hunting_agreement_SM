...
# -------- Script to perform the MSOM and verify the variable effects over species species detection  -----------
## -------- Load data -----------
...
sink(file = here("data","Richness.model.formula_r_int.txt"))
cat("model { 
     ...
     #likelihood - Ecological model for latent abundance of species i in sites j
      ...
      # population abundances.
		...
		# detection process model
				#r[j,i] <- 1/(1+exp(-(r0[i])))
				r[j,i] <- 1/(1+exp(-(r0[i] + r1[i]*Com.dist[j] + r2[i]*margem[j] + r3[i]*Com.dist[j]*margem[j])))
...
jags_data <- list(y=as.matrix(yaug),nspecies=nspecies, nzeros = nzeros, nSites = nSites , k = k,
                  Com.dist=Com.dist, margem=margem)#, Def=Def)
...
##----- perform the model ---- ##
out <- jags(jags_data, inits, params,  here("data","Richness.model.formula_r_int.txt"),
            n.chain=3, n.burnin=50000, n.iter=100000, n.thin=100)

##----- Save results ---- ## 
saveRDS(out, here("data", "RN_multitaxa_r_int.rds"))

##----- Load model ---- ##
out<-readRDS(here("data", "RN_multitaxa_r_int.rds"))
